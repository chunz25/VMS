...
";
	...
	foreach ($dataResult as $key1 => $value1) {
		...
		foreach ($dataResult2 as $key2 => $value2) {
			try {
				...
				$insert2['sir_no'] = $value2['sir_no'];
				$insert2['remark'] = $value2['remark'];
				$insert2['payment_date'] = setNull($value1['payment_date']);
				$insert2['payment_amount'] = setNol($value2['payment_amount']);

				$insertX2 = $db1->prepare($sqlInsertDetail);
				$insertX2->execute(array_values($insert2));
			} catch (PDOException $e) {
				// do something here
				print "Error!:  payment_item " . $e->getMessage() . "<br/> \n";
			}
		}
	}
	// clear var ---
	...
	try {
		$sqlToVmsDb = "call vms_db.insert_payment_sp()";
