...
foreach ($xx as $arr) {
	...
	$dataPOST = array("data" => array(array("ZUONR" => $arr["rs_no_sap"])));
	...
	// var_dump($dataResult);
	if (trim($dataResult[0]['msg']) != '') {
		$sqlUpdate = "UPDATE vms_db.invoice_receipt SET park_date='" . date("Y-m-d H:i:s") . "',park_msg='" . trim($dataResult[0]['msg']) . "' where rs_no_sap='" . $arr["rs_no_sap"] . "'";
