...
insert
into
	vms_gateway.goods_receive_cancel
(
	purchase_order_no,
	goods_receive_no,
	company_code,
...
insert
into
	vms_gateway.goods_receive_item_cancel
(
	goods_receive_no,
	line_item,
	product_code,
...
";
	...
	$sqlDelete = "call vms_db.delete_goods_receive_sp(?,?,?)";
	...
	foreach ($dataResult as $key1 => $value1) {
		try {
			...
			try {
				...
				print "Err \t delete_goods_receive_sp \t no : \t " . $value1["goods_receive_no"] . " \t " . $e->getMessage() . "\n";
			}
			...
			try {
				...
				// do something here
				print "Err \t goods_receive_cancel \t no : \t " . $value1["goods_receive_no"] . " \t " . $e->getMessage() . "\n";
			}
			...
			foreach ($dataResult2 as $key2 => $value2) {
				try {
					...
					print "Err \t goods_receive_item_cancel \t no : \t " . $value1["goods_receive_no"] . " \t " . $value2["product_code"] . " \t " . $e->getMessage() . "\n";
				}
			}
		} catch (PDOException $e) {
			// do something here
