						...
						if ($rs)
							while ($arr = $rs->FetchRow()) {
								...
								<tr valign="top">
									...
									<td align="right"><?= number_format($arr['unit_price'], 2); ?></td>
									<?php if ($data_header['revision_seq'] > 0) { ?>
		...
		<div class="col-xs-12">
			<div class="box-tools pull-left">
				...
				<a class="btn btn-default btn-flat btn-sm btn-info"
					onclick="cobayy('PROFORMA+INVOICE','400403','&param_menu4=2');"><i class="fa fa-edit"></i> <b>BACK
						TO LIST SETTLEMENT PRICE</b></a>
				<!-- button 2 ---------- -->
				...
				<a class="btn btn-default btn-flat btn-sm btn-default"
					onclick="bukaModalHelmizz301('#tempatmodal','index.php?main=040&main_act=010&main_id=400401_91&goods_receive_no=<?= urlencode($data_header['goods_receive_no']); ?>&po_no=<?= urlencode($data_header['purchase_order_no']); ?>','','#tampil4');"><i
						class="fa fa-print"></i> <b>PRINT GRN</b></a>

			</div>
			<div class="box-tools pull-right">
				<?php if ($_REQUEST["param_menu4"] == '33' || $_REQUEST["param_menu4"] == '34') { ?>
					<a class="btn btn-default btn-flat btn-sm  btn-warning"
						onclick="bukaModalHelmizz301('#tempatmodalTF2','index.php?main=040&main_act=010&main_id=400404_01_04&po_no=<?= urlencode($data_header['purchase_order_no']); ?>&gr_no=<?= urlencode($data_header['goods_receive_no']); ?>&ttl_amt=<?= urlencode($data_header['total_amount']); ?>&vat_amt=<?= urlencode($data_header['vat_amount']); ?>&grd_amt=<?= urlencode($data_header['grand_total']); ?>&npwp_no=<?= urlencode($data_header_supplier['npwp']); ?>&pfi_no=<?= urlencode($data_header['proforma_invoice_no']); ?>&status_pfi=<?= urlencode($data_header['status_pfi']); ?>','','#tampil2');"><i
							class="fa fa-edit"></i> <b>SEND INVOICE </b></a>
					<?php
				}
				if ($_REQUEST["param_menu4"] == '33') { ?>
					<!-- button 5 -------- -->
					<a class="btn btn-default btn-flat btn-sm btn-danger"
						onclick="dispute_process('Apakah Anda Yakin akan memproses dispute price.... ?','DISPUTE+PRICE','400403_02_03','<?= $_REQUEST["param_menu3"]; ?>')"><i
