		...
		<div class="col-xs-12">
			...
			<div class="box-tools pull-right">
				...
				if (in_array(date('d'), $tgl) || in_array(date('D'), $day) || in_array(date("Y-m-d"), $tglmerah)) {
					...
					?>
					<a class="btn btn-default btn-flat btn-sm  btn-warning"
						onclick="bukaModalHelmizz301('#tempatmodalTF','index.php?main=040&main_act=010&main_id=400404_01_04&po_no=<?= urlencode($data_header['purchase_order_no']); ?>&gr_no=<?= urlencode($data_header['goods_receive_no']); ?>&ttl_amt=<?= urlencode($data_header['total_amount']); ?>&vat_amt=<?= urlencode($data_header['vat_amount']); ?>&grd_amt=<?= urlencode($data_header['grand_total']); ?>&npwp_no=<?= urlencode($data_header_supplier['npwp']); ?>&pfi_no=<?= urlencode($data_header['proforma_invoice_no']); ?>&status_pfi=<?= urlencode($data_header['status_pfi']); ?>','','#tampil2');">
						<i class="fa fa-edit"></i>
						<b>CONFIRM & SEND INVOICE </b></a>
				...
				<a class="btn btn-default btn-flat btn-sm btn-danger"
					onclick="dispute_process('Apakah Anda Yakin akan memproses dispute price.... ?','PROFORMA+INVOICE','400403_01_03','<?= $_REQUEST["param_menu3"]; ?>')"><i
