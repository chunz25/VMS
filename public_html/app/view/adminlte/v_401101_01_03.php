        ...
        <section class="invoice">
		 <form role="form" id="my_form" action="index.php" method="post" enctype="multipart/form-data">
          ...
          <div class="row">
			...
			$qty_rev1 = ($arr['qty_rev1']>=0) ? "<h4><button class='btn btn-warning btn-xs btn-flat'>".number_format($arr['qty_rev1'])."</button></h4>" : number_format($arr['qty_ori']);
					$qty_rev2 = ($arr['qty_rev2']>=0) ? "<h4><button class='btn btn-warning btn-xs btn-flat'>".number_format($arr['qty_rev2'])."</button></h4>" : number_format($arr['qty_finish']);
					$qty_rev3 = ($arr['qty_rev3']>=0) ? "<h4><button class='btn btn-warning btn-xs btn-flat'>".number_format($arr['qty_rev3'])."</button></h4>" : number_format($arr['qty_finish']);
					$qty_rev4 = ($arr['qty_rev4']>=0) ? "<h4><button class='btn btn-warning btn-xs btn-flat'>".number_format($arr['qty_rev4'])."</button></h4>" : number_format($arr['qty_finish']);
					$qty_rev5 = ($arr['qty_rev5']>=0) ? "<h4><button class='btn btn-warning btn-xs btn-flat'>".number_format($arr['qty_rev5'])."</button></h4>" : number_format($arr['qty_finish']);
					$qty_rev6 = ($arr['qty_rev6']>=0) ? "<h4><button class='btn btn-warning btn-xs btn-flat'>".number_format($arr['qty_rev6'])."</button></h4>" : number_format($arr['qty_finish']);
			?>
					  <tr valign="top">					
						...
						<?php } else { ?>
						<td align="right"><?= number_format($arr['qty_ori']);?>
						<input type="hidden" name="qty_rev[<?= $arr['product_code'];?>]" value="">
						</td>
						<?php } ?>
					  </tr>
				<?php } ?>
...
<hr>
            ...
            <div class="col-xs-12">
				<div class="box-tools pull-right">
					<a class="btn btn-default btn-flat btn-sm btn-info"  onclick="cobayy('DISPUTE+QUANTITY','401101','<?= $_REQUEST["param_menu3"];?>');"><i class="fa fa-edit"></i> <b>BACK TO LIST GR</b></a>
